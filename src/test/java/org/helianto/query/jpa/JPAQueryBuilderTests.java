...
public class JPAQueryBuilderTests {
	...
	public void buildTwoFields() {
		...
		assertEquals("select alias from TestingDomainObject where alias.FIELD = '1' AND alias.OTHER = '2'"
				, query.build());
	}
	...
	public void buildOrFields() {
		...
		assertEquals("select alias from TestingDomainObject where alias.FIELD = '1' OR alias.OTHER = '2'"
				, query.build());
	}
	...
	public void buildLike() {
		...
		assertEquals("select alias from TestingDomainObject where alias.FIELD like '%TEXT%'"
				, query.build());
	}
	...
	public void buildChar() {
		query.eq("FIELD", 'X');
		assertEquals("select alias from TestingDomainObject where alias.FIELD = 'X'"
				, query.build());
	}
	...
	public void buildOrderBy() {
		...
		assertEquals("select alias from TestingDomainObject order by alias.FIELD"
				, query.build());
	}
	...
	public void buildOrderByDesc() {
		...
		assertEquals("select alias from TestingDomainObject order by alias.FIELD ASC, alias.OTHER DESC"
				, query.build());
	}
